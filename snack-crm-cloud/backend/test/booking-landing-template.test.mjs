...
test("approved public booking pages are connected at their real launch URLs", async () => {
  ...
  assert.match(landingScript, /\/api\/public\/classes/);
  ...
  assert.match(schedulerScript, /\/api\/public\/class-registrations/);
  assert.match(schedulerScript, /\/api\/public\/classes\//);
  assert.match(schedulerHtml, /Food Restrictions/);
  assert.match(schedulerHtml, /<h2 id="family-information-title">Family Information<\/h2>/);
  assert.doesNotMatch(schedulerHtml, /Tell us who is coming/);
  assert.match(schedulerHtml, /Mobile Phone/);
  assert.match(schedulerHtml, /First and Last Name/);
  assert.match(schedulerHtml, /Street, City, State, ZIP/);
  assert.match(schedulerHtml, />Phone Call</);
  assert.match(schedulerHtml, /id="summary-location"/);
  assert.match(schedulerScript, /normalizeMailingAddress/);
  assert.match(schedulerScript, /bookingScheduling\.clinicLocation/);
  assert.match(schedulerScript, /activeClassSession\.location/);
